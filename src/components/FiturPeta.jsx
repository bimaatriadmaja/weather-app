...
const FiturPeta = () => {
  ...
  const [map, setMap] = useState(null); // State untuk peta
  const [marker, setMarker] = useState(null); // State untuk marker
  ...
  useEffect(() => {
    if (!map) {
      const newMap = L.map('map').setView([0, 0], 2); // Set view awal ke koordinat (0, 0) dengan zoom level 2
  ...
  // Update marker saat data cuaca berubah
  useEffect(() => {
    if (map && weatherData) {
      ...
      if (marker) {
        ...
        const newMarker = L.marker([lat, lon]).addTo(map); // Tambahkan marker baru
        setMarker(newMarker);
      }
      map.setView([lat, lon], 10); // Pindahkan view peta ke lokasi baru
    }
  }, [weatherData, map, marker]);
  ...
  const shareWeather = () => {
    if (weatherData) {
      const shareText = `Current weather in ${weatherData.name}: ${weatherData.main.temp}°C, ${weatherData.weather[0].description}. Check it out!`;
    ...
    <div className={`min-h-screen ${darkMode ? 'bg-gray-900 text-white' : 'bg-gradient-to-br from-blue-400 to-purple-500 text-gray-800'} flex items-center justify-center p-4`}>
      <div className={`${darkMode ? 'bg-gray-800 text-white' : 'bg-white/90'} backdrop-blur-md rounded-xl shadow-2xl w-full max-w-md p-6`}>
        ...
        <button
          ...
          className={`w-full p-3 ${darkMode ? 'bg-green-600' : 'bg-green-500'} text-white rounded-lg hover:bg-green-600 transition-colors mb-6`}
        >
        ...
        {loading ? (
          ...
          <div className="text-center">
            ...
            <button
              ...
              className={`w-full p-3 ${darkMode ? 'bg-purple-600' : 'bg-purple-500'} text-white rounded-lg hover:bg-purple-600 transition-colors mt-6`}
            >
        ...
        <div id="map" className="mt-6 h-64 rounded-lg"></div>
      </div>
    </div>
  );
};

export default FiturPeta;